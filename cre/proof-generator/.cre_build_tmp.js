...
var configSchema = exports_external.object({
  ...
  finetuneProofRegistryAddress: exports_external.string(),
  modelRegistryAddress: exports_external.string(),
  gasLimit: exports_external.string()
});
var FinetuneProofRegistryABI = [
  {
    inputs: [
...
var verifyModelExists = (runtime2, evmClient, modelRegistryAddress, modelId) => {
  ...
  runtime2.log(`getModel raw response: ${hexData.substring(0, 40)}... (${result.data.length} bytes)`);
  if (!hexData || hexData === "0x" || result.data.length === 0) {
    runtime2.log(`Model "${modelId}" — contract returned empty data (simulation mode), skipping verification`);
    return;
  }
  ...
  runtime2.log(`Model "${modelId}" verified — owner ${model.owner}`);
};
var computeProofHash = (modelId, timestampMs2, sessionHashes) => {
  const encoded = encodeAbiParameters([
    { type: "string" },
    { type: "uint256" },
    { type: "bytes32[]" }
  ], [modelId, BigInt(timestampMs2), sessionHashes]);
  return keccak256(encoded);
};
var submitProof = (runtime2, evmClient, registryAddress, modelId, timestampMs2, sessionHashes, gasLimit) => {
  const callData = encodeFunctionData({
    abi: FinetuneProofRegistryABI,
    functionName: "storeProof",
    args: [modelId, BigInt(timestampMs2), sessionHashes, "0x"]
  });
  const reportResponse = runtime2.report(prepareReportRequest(callData)).result();
  runtime2.log(`DON consensus report received — submitting on-chain`);
  ...
  if (writeResult.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`Failed to write proof on-chain: ${writeResult.errorMessage || writeResult.txStatus}`);
  }
  const txHash = writeResult.txHash ? bytesToHex(writeResult.txHash) : "0x" + "0".repeat(64);
  runtime2.log(`storeProof transaction succeeded — txHash: ${txHash}`);
  return {
    proofId: txHash,
    txHash
  };
};
var onProofRequested = (runtime2, payload) => {
  const inputText = Buffer.from(payload.input).toString("utf-8");
  const { modelId, sessionHashes, timestampMs: timestampMs2 } = JSON.parse(inputText);
  if (!modelId || !sessionHashes || sessionHashes.length === 0 || !timestampMs2) {
    throw new Error("Invalid payload: modelId, sessionHashes (non-empty), and timestampMs are required");
  }
  runtime2.log(`Proof requested for model "${modelId}" with ${sessionHashes.length} session(s)`);
  ...
  const modelRegistryAddress = config.modelRegistryAddress;
  const proofRegistryAddress = config.finetuneProofRegistryAddress;
  verifyModelExists(runtime2, evmClient, modelRegistryAddress, modelId);
  const typedSessionHashes = sessionHashes.map((h) => h);
  const proofHash = computeProofHash(modelId, timestampMs2, typedSessionHashes);
  runtime2.log(`Proof hash computed: ${proofHash}`);
  const { proofId, txHash } = submitProof(runtime2, evmClient, proofRegistryAddress, modelId, timestampMs2, typedSessionHashes, config.gasLimit);
  runtime2.log(`Proof generation complete: ${safeJsonStringify({ proofId, txHash, modelId })}`);
